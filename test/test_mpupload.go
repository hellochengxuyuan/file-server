...
func main() {
	username := "zhaogongzuo"
	token := "86e0e077ff358f3cb24b1510da142048e35e8ba5"
	filehash := "1847b1e6348be833bb7708a80fad82d9278a2b4f"
	...
	fmt.Println(uploadID, 123456789)
	chunkSize := jsonit.Get(body, "data").Get("ChunkSize").ToInt()
	fmt.Println(chunkSize, 123456789)

	fmt.Printf("uploadid: %s  chunksize: %d\n", uploadID, chunkSize)

	// 3. 请求分块上传接口
	filename := "/home/pkg/sogoupinyin_2.2.0.0108_amd64.deb"
	tURL := "http://localhost:8080/file/mpupload/uppart?" +
		"username=zhaogongzuo&token=" + token + "&uploadid=" + uploadID
